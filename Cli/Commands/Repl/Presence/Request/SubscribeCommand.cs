...
public class SubscribeCommand : Command
{
  ...
  {
    Description = "The JID to send a subscription request to",
    Arity = ArgumentArity.ExactlyOne
  };

  public SubscribeCommand(IXmppClient client) : base("subscribe", "Request a presence subscription from a JID")
  {
    _client = client;
  ...
  private async Task CommandHandler(ParseResult parseResult)
  {
    ...
    var result = await im.RequestPresenceSubscription(jid);
    if (result.IsT0) return;
    
    var err = (IClientError) result.Value;
    AnsiConsole.MarkupLine($"[bold red]An error occured while requesting the presence subscription: {err.What()}[/]");
  }
}