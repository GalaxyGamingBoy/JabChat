...
public class CancelCommand : Command
{
  ...
  {
    Description = "The JID to cancel a presence request from",
    Arity = ArgumentArity.ExactlyOne
  };

  public CancelCommand(IXmppClient client) : base("cancel", "Cancel a presence request from a JID")
  {
    _client = client;
  ...
  private async Task CommandHandler(ParseResult parseResult)
  {
    ...
    var result = await im.CancelPresenceSubscription(jid);
    if (result.IsT0) return;
    
    var err = (IClientError) result.Value;
    AnsiConsole.MarkupLine($"[bold red]An error occured while cancelling the presence subscription: {err.What()}[/]");
  }
}